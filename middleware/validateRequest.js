	...
	if(!token) {
		...
		try {
			...
			if(err.name === 'TokenExpiredError'){
				...
				Users.findById(tokenDecoded.userid)
					.then(userExpired => {
						if(userExpired){
			...
			.then((user) => {
				if (user) {
					...
					if (  (url.indexOf('admin') !== -1 && dbUserObj.roles.isAdmin) ||
						...
						(url.indexOf('sales') !== -1 && dbUserObj.roles.isSales) ||
						(url.indexOf('finance') !== -1 && dbUserObj.roles.isFinance) ||
						(url.indexOf('customer') !== -1 && dbUserObj.roles.isCustomer) ||
						(url.indexOf('audit') !== -1 && dbUserObj.roles.isAudit) ||
						(url.indexOf('admin') === -1 &&
							url.indexOf('business') === -1 &&
							url.indexOf('sales') === -1 &&
							url.indexOf('finance') === -1 &&
